    ...
    def run(self):
        ...
        while True:
            ...
            if self.recv_msg != None:
                try:
                    if self.recv_msg.get('msg_type') == "capture":
                        print("capture")
                        self.recieve_order(order_enum.CAPTURE)
                    elif self.recv_msg.get('msg_type') == "saving":
                        print("saving")
                        print(self.recv_msg.get('msg_content'))
                        self.recieve_order(order_enum.SAVING)
                    elif self.recv_msg.get('msg_type') == "ongoing":
                        print("ongoing")
                        self.recieve_order(order_enum.ONGOING)
                    elif self.recv_msg.get('msg_type') == "closing":
                        print("closing")
                        self.recieve_order(order_enum.CLOSING)
                    else:
